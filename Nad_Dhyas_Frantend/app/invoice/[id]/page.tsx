...
export default function InvoicePage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        ...
        async function fetchInvoice() {
            try {
                console.log('🔍 Fetching invoice for student ID:', params.id);
                
                // Add timestamp to prevent caching
                const timestamp = new Date().getTime();
                const url = `/api/invoice/${params.id}?t=${timestamp}`;
    ...
    if (loading) return (
        <div style={{ padding: '50px', textAlign: 'center', fontFamily: 'Arial, sans-serif' }}>
            <div style={{ fontSize: '18px', color: '#6b7280', marginBottom: '20px' }}>Loading Invoice...</div>
            <div style={{ fontSize: '14px', color: '#9ca3af' }}>Student ID: {params.id}</div>
        </div>
    );
    ...
    if (!data) return (
        <div style={{ padding: '50px', textAlign: 'center', maxWidth: '600px', margin: '0 auto', fontFamily: 'Arial, sans-serif' }}>
            ...
            <div style={{ color: '#6b7280', marginBottom: '30px', fontSize: '16px' }}>
                Unable to find invoice for Student ID: <strong>{params.id}</strong>
            </div>
            ...
            <div style={{ 
                ...
                <ul style={{ margin: 0, paddingLeft: '20px', color: '#374151', lineHeight: '1.8' }}>
                    <li>Student ID is correct</li>
                    <li>Student exists in the database</li>
